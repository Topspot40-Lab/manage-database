...
from typing import Optional, Any, Dict, List  # ✅ FIX: add Any, Dict, List
...
# ✅ keep your new modular helpers
from backend.services.collections_normalize import (
    to_snake_track, normalize_artist_collab,
    finalize_triplet_fields, harmonize_mode_and_feature, build_artist_display_name
)
from backend.services.collections_artists import (
    build_artist_table_from_tracks, backfill_featured_artist_ids_from_enriched_table
)
from backend.services.collections_serialize import serialize_collection_output


# ✅ FIX: this model is referenced by the endpoint; define it here
...
# ✅ utility: temporarily flip config flags safely
@contextmanager
def temp_flags(**kw):
    ...
    try:
        ...
        for c in raw_candidates:
            if not isinstance(c, dict):
                continue
            title = (c.get("title") or c.get("trackName") or c.get("track") or c.get("name") or "").strip()
            artist = (c.get("artist") or c.get("artistName") or "").strip()
            year = None
            yv = (c.get("year") or c.get("yearReleased"))
            if yv:
                try:
                    y = int(str(yv)[:4])
                    if 1600 <= y <= 2100:
        ...
        tracks = [normalize_artist_collab(to_snake_track(t)) for t in tracks]
        ...
        for spare in list(spares):
            if len(tracks) >= request.target_count:
                break
            s_norm = normalize_artist_collab(to_snake_track(spare))
            enriched_list = enrich_tracks_with_spotify([s_norm], is_test_mode=False)
            if enriched_list and enriched_list[0].get("spotify_track_id"):
                enriched_list[0]["rank"] = len(tracks) + 1
                tracks.append(enriched_list[0])

        # STEP 8: re-rank + ranking object
        reassign_ranks(tracks)
        ranking = build_ranking_from_tracks(tracks)
        ok, msg = validate_compact_ranks(ranking)
        if not ok:
            logger.warning("Ranking validation: %s", msg)
        if max_step == 8:
            return {"message": "Stopped after STEP 8"}

        # STEP 8.5: artist table (+ featured) → enrich IDs/artwork
        artist_tbl = build_artist_table_from_tracks(tracks)
        artist_tbl = enrich_artist_table_with_spotify(
            artist_table=artist_tbl,
            tracks_after_step3=tracks,
            fill_missing_ids_via_search=True
        )

        # STEP 9: XAI descriptions (intros/details/artists)
        flag_kwargs: Dict[str, bool] = {}
        if enable_rank_intro is not None:
            flag_kwargs["ENABLE_RANK_INTRO"] = enable_rank_intro
        if enable_track_detail is not None:
            flag_kwargs["ENABLE_TRACK_DETAIL"] = enable_track_detail
        if enable_artist_detail is not None:
            flag_kwargs["ENABLE_ARTIST_DETAIL"] = enable_artist_detail
        ...
        # ✅ FIX: proper context manager usage for temp flags
        if flag_kwargs:
        ...
        # STEP 9.5: finalize triplet & harmonize fields
        finalize_triplet_fields(tracks)
        for t in tracks:
            harmonize_mode_and_feature(t)
            build_artist_display_name(t)

        # STEP 9.6: backfill featured_artist_id from enriched artist table
        backfill_featured_artist_ids_from_enriched_table(artist_tbl, tracks)

        # STEP 10: serialize
        out = serialize_collection_output(
            slug=slug,
            display_name=theme,
