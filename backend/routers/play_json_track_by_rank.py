...
def load_json_track_file_endpoint(
    ...
    file_path = BASE_DIR / "data/json_files/genredecade" / decade / filename

    if not file_path.exists():
        raise HTTPException(status_code=404, detail=f"File not found: {file_path}")

    # 🧠 Only reload if forced or not already cached
    if force_reload or "tracks" not in track_cache or track_cache.get("filename") != filename:
    ...
    return {
        ...
        "filename": filename,
        "tracks": track_cache["tracks"]  # ✅ full track list
    }
