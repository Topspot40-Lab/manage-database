...
async def safe_play(kind: str, bucket: str, key: str) -> bool:
    """
    Robust MP3 playback from Supabase with HEAD check and GET retries.
    Returns True on success, False on missing/failure.
    ...
    # Fast existence probe to avoid long GET timeouts for missing objects
    try:
        ...
        # Not fatal—some gateways don’t support HEAD properly; we’ll try GET anyway.
    ...
    for attempt in range(1, _SUPA_FETCH_RETRIES + 1):
        ...
        try:
            # play_mp3(bucket, key) fetches (GET) + blocks until ffplay exits
            rc = await play_mp3(bucket, key)
            dt = time.perf_counter() - t0
            if rc == 0:
                logger.info("✅ %s MP3 played in %.2fs (%s/%s)", kind, dt, bucket, key)
                return True
